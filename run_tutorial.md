...
- `fedmed/pl-superlink:0.0.8` – spins up a Flower **SuperLink** and launches the bundled FedMed **ServerApp**.
- `fedmed/pl-supernode:0.0.8` – starts a Flower **SuperNode** that executes the FedMed **ClientApp**.
...
```
✔ docker.io/fedmed/pl-superlink:0.0.8 http://chris:8000/api/v1/plugins/6/
✔ docker.io/fedmed/pl-supernode:0.0.8 http://chris:8000/api/v1/plugins/7/
```
...
1. Open the `superlink` analysis, right-click its lone node, and select **Add Pipeline → FedMed Flower SuperLink v0.0.8**.
...
1. Open the `supernode` analysis, right-click the node, and select **Add Pipeline → FedMed Flower SuperNode v0.0.8**.
...
2. Remove the OCI images if you no longer need them:
   ```bash
   docker image rm docker.io/fedmed/pl-superlink:0.0.8 docker.io/fedmed/pl-supernode:0.0.8
   ```
